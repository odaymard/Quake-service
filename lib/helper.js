 ...
 * @param {Number} topCloseQuakes  The number of earthquakes to show, in our example it is 10, so we want to list 10 earthquakes that happened in the closest proximity  ascending.
...
function getTopCloseQuakes(quakesArray, inputCoord, topCloseQuakes) {
  ...
  for (let i = 0; i < quakesArray.length; i++) {

    const quakeCoord = [quakesArray[i][1][1], quakesArray[i][1][0]]; //Hint: in the API, the coordinate array starts with long then lat. 
    const distanceToPoint = getDistanceFromLatLonInKm(inputCoord, quakeCoord);
    const title = quakesArray[i][0];
...
function insertIntoSortedArray(sortedArray, dataToInsert, top) {
  ...
  sortedArray.splice(insertionIndex, 0, { distance: (distanceToPoint), title: title });
